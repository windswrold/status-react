...
(defn switcher-button []
  {:width      constants/switcher-button-size
   ...
   :opacity    animation/switcher-button-opacity})
...
(defn switcher-button-touchable []
  (merge-switcher-button-common-styles
   {:align-self :center
    :bottom     (constants/switcher-bottom-position)}))

(defn switcher-close-button-background []
  (merge-switcher-button-common-styles
   {:background-color colors/switcher-background
    :opacity          animation/switcher-close-button-background-opacity}))

(defn switcher-close-button-icon []
  (merge-switcher-button-common-styles
   {:opacity          animation/switcher-close-button-icon-opacity}))

(defn switcher-screen []
  (let [radius @animation/switcher-screen-radius
        bottom (- (constants/switcher-center-position) radius)
        size   (* 2 radius)]
    (merge-switcher-button-common-styles
...
(defn switcher-screen-container []
  (let [radius                 @animation/switcher-screen-radius
        bottom                 (- radius (constants/switcher-center-position))
